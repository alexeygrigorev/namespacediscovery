...
A \emph{namespace}, in general, refers to a collection of identifiers that are grouped together, because they share functionality or purpose.
%TODO: You need to cite this statement.
In computer science, namespaces are typically used for providing modularity and for resolving name conflicts.
%TODO: Same problem here.
A popular example is XML,
%TODO: Why is this a popular example? Maybe just example
where elements and attributes are prefixed to ensure uniqueness and remove ambiguity between them \cite{source:xml_bray99}.
For example, a tag \texttt{<table>} may refer to tabular data or to a piece of furniture, depending on the context. To distinguish between them we prefix the identifiers with namespaces and have \texttt{<h:table>} for hypertext and \texttt{<f:table>} for furniture.
%TODO: This is an excellent example for you thesis text. But I think we don't need that in the introduction in this document that will be read by cs-experts only.
Another good example is the Java programming language, where packages give a way of organizing identifiers into namespaces \cite{source:java_gosling14}. For example, \texttt{List} may refer to a collection of objects, or to a component of the user interface library. To solve this ambiguity, we have to use fully qualified names like \texttt{java.util.List} and \texttt{java.awt.List}.
%TODO: Same here. One setence that namespaches are used in XML and Java with the two sources would be sufficient.
So namespaces provide means of modularity for making it possible to distinguish between identifiers with exactly the same name, but different semantics.
%TODO: Is that a conclusion? You can not derive general statements from examples.

We can extend the notion of namespaces to mathematical formulae.
%TODO: Point out what you are doing. In this thesis, we will extend...
 Even though formally, from the first order logic point of view, the identifier names can be chosen arbitrarily. However, there are common mathematical notations that are used by different research communities. 
%TODO: This need to be rewritten. Maybe you can intorduce the definition of a formula in FOL and cite references from the logic course you had in your bachelor studies. Second, distingutush the kind of formula we are going to investigate that are more like natural language expressions. I would keep
For example, in physics it is common to write the energy-mass relation as $E=mc^2$ rather than $x=yz^2$.
%I'd keep this example, in contrast to the concrete XML and java examples (MS)
...
However, the same identifier may be used in different areas but denote different things: For example, $E$ may stand for ``energy", ``expected value" or ``elimination matrix", depending on the domain of the article. Thus, we can say that these identifiers form namespaces, and we refer to such namespaces as \emph{identifier namespaces}.
%TODO: We should give this process a name. Maybe namespace disambiguation. Also I guess there is related works that does similar things. Maybe this https://www.lrde.epita.fr/dload/20080116-Seminar/durlin-disamb.pdf (But related works is a part of the thesis not of the proposal)

There could be several approaches for discovering identifier namespaces. One way is to assume that there is a strong correlation between identifiers in a document and the namespace of the document, and this correlation can be exploited to categorize documents and thus discover namespaces.
...
%TODO: This is also some text that could be used in the thesis itself. You can just describe the example with natural langute for this proposal.

\subsection{Related Work}

Kristianto et al \cite{source:kristianto14} highlight the importance of interlinking the scientific documents and in their study they do it through annotating mathematical formulas and finding the documents that share the same identifiers. Sch\"oneberg et al \cite{source:schoneberg14} propose mathematical-aware part of speech tagger and they discuss how it can be applied for classifying scientific publications.

There are several researches related to extracting textual description of mathematical formulas. One of the earliest works is by Grigore et al \cite{source:grigore09} that focuses on disambiguation,
...
Some work is also done in clustering mathematical formulas by Ma et al \cite{source:ma10} to facilitate formula search where they propose features that can be extracted from the formulas.
...
Furthermore, we believe that there is a link between document categorization and namespace discovery.
%TODO: That does not sound scientific. I'd remove the sentence here.
 However, not all categorization techniques are equally good, thus in order for a category to make a good namespace, 
%TODO: You need to cite that or remove the statement. I'd just write
We aim to find 
 %it should be 
 \emph{meaningful}
 namespaces%
, in the sense that it can be related to a real-world area of knowledge, such as physics, linear algebra or statistics.
...
%TODO: This has already beend said?
% Additionally, we believe that formulas comprise an integral part of a mathematical corpus, 
% namespaces should be discovered based on these formulas

Once such namespaces are found, they can give good categorization of scientific documents based on formulas and notation used in them.
%TODO: We need more explaination here. We assume that different formula of a document use identifiers from the same namespace. Thus we can generalize from the namespace of the formula to a namespace of the document. This namespace can be regared as categorization (or maybe classification) of the document.
...
We also see that using namespaces is beneficial for relating identifiers to definitions.
%TODO: Maybe expect istead of see fits better here.
 Thus, as an application of namespaces, we would like to be able to use them for better definition extraction. It may help to overcome some of the current problems in this area, for example, the problem of \emph{dangling identifiers}
%TODO: If you took the definition of the wolska thesis you should add a citation here 
  - identifiers that are used in formulas but never defined in the document.
%TODO: Here you mean the category? We should keep the anlogy to comuter sciecne, where commands like "import" or "declare default namesapce" are used to keep the notation short.
...
\end{enumerate}
%TODO: Add more description here. And keep focussed on the goals. For example "To incorporate the found namespaces to the existing MLP framework" reads like a programming and not like a research task. The significant research output is an improvement w.r.t precision and recall compared to the current MLP system baseline.
...
The data set that we plan to use is a subset of English wikipedia articles - all those that contain the \texttt{<math>} tag. The textual dataset can potentially be quite big: for example, the English wikipedia contains 4.5 million articles, and many thousands of them contain mathematical formulas. This is why it is important to think of ways to parallelize it. Thus, the algorithms will be implemented in Apache Flink \cite{source:flink}. Additionally, wikipedia articles are already categorized and this may be useful for evaluating the quality of namespaces. 
...
\end{enumerate}
%TODO: Is that list required? If not delete it. There is only one deliverable, the master theisis. Don't promise too much in the bginning.
