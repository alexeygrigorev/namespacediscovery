...
Give important implementation details
...
``$\lceil$''. %Ideally, there symbols should be represented by \verb|<mo>| tags, 
%but there are many cases when they are not.
...
\textbf{TODO put a table to compare English vs Russian}
...
The more an identifier occurs, the more is its score. If an identifier occurs 
3 times, then it most likely will have a score around 2.5-3, and this 
gives us more confidence in the correct definition of an identifier than 
a score of less than 1 - because it corresponds to an identifier that 
occurred just once. To compare confidence scores we can apply an additional 
transformation to normalize the scores to the $[0, 1]$ scale. 
To do this we can apply the hyperbolic tangent function: it is zero at 0 
and it is almost 1 around 5. However, the usual tanh is quite steep, 
if something occurred just 3 times it is not enough to give it a score of
0.99, therefore we use tanh(x/2).

Score normalization to $[0, 1]$ scale: use tanh (see fig.~\ref{fig:tanh})
Usual tanh is too steep: tanh(2) is 0.95 and tanh(3) is 0.99, so we use 
tanh(x/2) instead, and it will give  tanh(3/2) = 0.90

tanh is monotonous therefore larger values of transformed ... correspond to 
larger values of ...
