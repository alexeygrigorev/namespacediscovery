...
\caption{$K$ in $K$-Means vs overall purity of clustering: the purity increases linearly with $K$ ($R^2 = 0.99$)}
...
structure. It is hard to fully automatically, and we choose to use 
...
are merged into one. Then each found namespace is mapped to 
the most suitable second-level category.


However these categories are only good for the English wikipedia and 
...
in Russian  \cite{feodosimov2000grnti}. The dataset is retrieved from the 
official website \url{http://grnti.ru/} \cite{grntiweb}.
