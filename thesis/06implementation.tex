...
It is also interesting to look at all the occurrences of ``$\mu$'' in 
...
\end{subtable}
\caption{A namespace-defining cluster about Measure Theory discovered by $K$-Means
with $K=9500$ and SVD with $k=250$.}
...
observe that it grows linearly with $k$, and it appears that there is rather 
...
\hline
\end{tabular}
\caption{Some of definitions of ``$\lambda$'' in the clustering obtained with $K$-Means.}
...
\begin{tabular}{|c|c|c|}
  ...
  \hline
Astronomical catalogues & 53 & 0.9811 \\
Set theory & 10 & 0.9000 \\
Propositional calculus & 10 & 0.8000 \\
Group theory & 10 & 0.8000 \\
Electromagnetism & 9 & 0.8889 \\
Rotation & 9 & 0.8889 \\
Numerical analysis & 9 & 0.8889 \\
Algebraic geometry & 9 & 0.8889 \\
Thermodynamics & 8 & 0.8750 \\
Coordinate systems & 8 & 1.0000 \\
Numerical analysis & 8 & 1.0000 \\
Quantum mechanics & 8 & 0.8750 \\
Group theory & 8 & 0.8750 \\
Fluid dynamics & 7 & 0.8571 \\
Financial economics & 7 & 0.8571 \\
\hline
\end{tabular}
\caption{Top namespace-defining clusters discovered by $K$-Means with $K=8250$ on 
LSA-space with $k=500$.}
\label{tab:strong-kmeans-svd}
\end{table}
...
\begin{tabular}{|c|c|}
  ...
  Article & Identifiers \\
  \hline
Skew-symmetric matrix &  $\Sigma_a,\lambda_1,\theta,\lambda_2,\pi,B,\lambda_r,...$ \\
Diagonalizable matrix & $v_1,\lambda_1,v_k,\lambda_3,\lambda_2,\lambda_i,\lambda_k,\lambda_j,\lambda_n,...$ \\
Eigenvalues and eigenvectors & $v_i,\mu_A,\lambda_i,d,\lambda_n,...$ \\
Principal axis theorem & $v_1,u_1,\lambda_1,\lambda_2,D,S,u,...$ \\
Generalized eigenvector & $m,V,\lambda,\lambda_2,\lambda_1,\beta,k,...$ \\
Linear independence &  $\Lambda,v_j,u_2,v_3,u_n,\lambda_1,\lambda_3,\lambda_2,...$ \\
Symmetric matrix &  $\Lambda,\lambda_1,\lambda_2,D,Q,P,\lambda_i,...$ \\
\hline
\end{tabular}
\caption{Articles in the cluster ``Linear Algebra''.}
\label{tab:soft-kmeans-la}
\end{subtable}% mask EOL
\begin{subtable}{0.3\textwidth}
\centering
...
$t$ & real argument & 0.99 \\
$u$ & eigenvalues & 0.89 \\
$\Lambda$ & nonzero vector & 0.99 \\
$\theta$ & integer multiple & 0.83 \\
$\lambda$ & eigenvalue & 3.51 \\
$\lambda_1$ & eigenvalues & 4.40 \\
$\lambda_2$ & eigenvalues & 2.60 \\
$\lambda_i$ & complex number & 0.89 \\
$\lambda_k$ & eigenvalues & 0.99 \\
\hline
\end{tabular}
\caption{Definitions in ``Linear Algebra''.}
\label{tab:soft-kmeans-la-def}
\end{subtable}
\caption{A ``Linear Algebra'' cluster found by $K$-Means with $K=8250$ and $k=500$.}
\label{tab:strong-kmeans-lsa}
\end{table}
...
\begin{tabular}{|c|c|c|c|}
  ...
  Size & Namespace Name & Definition & Score \\
  \hline
5 & Analytic geometry & odd multiple & 0.89 \\
4 & Analytic number theory & modular lambda function & 1.88 \\
3 & Applied and interdisciplinary physics & wavelength & 3.57 \\
4 & Cartographic projections & longitude & 6.36 \\
5 & Cartography & longitude & 6.37 \\
5 & Chemical kinetics & rate & 0.83 \\
3 & Complex analysis & forces & 0.93 \\
8 & Coordinate systems & longitude & 7.99 \\
6 & Electromagnetic radiation & wavenumber & 1.86 \\
3 & Electromagnetic spectrum & wavelength & 0.99 \\
6 & Electronic amplifiers & typical value & 0.93 \\
5 & Electronic engineering & wavelength & 0.89 \\
7 & Fluid dynamics & wavelength & 4.63 \\
6 & Group theory & limit ordinal & 0.97 \\
3 & Infinity & limit ordinals & 2.68 \\
4 & Lagrangian mechanics & lagrange & 0.89 \\
3 & Large numbers & ordinals & 1.80 \\
7 & Linear algebra & eigenvalue & 3.51 \\
4 & Materials science & wavelength & 0.89 \\
5 & Mathematical analysis & shape & 1.78 \\
4 & Mathematical physics & measure & 0.86 \\
5 & Measure theory & lebesgue measure & 0.95 \\
6 & Mechanics & previous expression & 0.95 \\
3 & Multilinear algebra & natural isomorphisms & 0.84 \\
3 & Number theory & liouville function & 0.93 \\
6 & Operator theory & identity function & 0.86 \\
3 & Optics & wavelength & 1.70 \\
3 & Philosophy of physics & kinds & 0.81 \\
7 & Physical constants & wavelength & 0.89 \\
6 & Polarization & length & 1.85 \\
5 & Probability distributions & exponential distribution & 1.79 \\
4 & Probability distributions & shape parameter & 0.93 \\
3 & Probability theory & suitable choice & 0.83 \\
3 & Quantum field theory & coupling constant & 1.94 \\
3 & Quantum information science & state & 2.66 \\
4 & Quantum mechanics & wavelength & 0.99 \\
5 & Radio technology & wavelength & 2.80 \\
7 & Rubber properties & engineering strain & 8.96 \\
3 & Statistical theory & single number & 0.99 \\
3 & Statistics & rate & 1.97 \\
3 & Theoretical physics & eigenvalues & 1.91 \\
4 & Transforms & eigenvalues & 1.82 \\
3 & Wireless & wavelength & 1.90 \\
\hline
\end{tabular}
\caption{Some of definitions of ``$\lambda$'' in the clustering obtained with $K$-Means.}
\label{tab:strong-kmeans-lambda}
\end{table}


Like for the weak association case, let us consider a cluster with documents on 
Linear Algebra  (see table~\ref{tab:soft-kmeans-la}). We can observe that it 
overlaps with the Linear Algebra discovered previously: there are also 
``Diagonalizable matrix'', ``Eigenvalues and eigenvectors''
``Principal axis theorem'', ``Linear independence'' and ``Symmetric matrix'' 
articles. The extracted definitions are also similar (see table~\ref{tab:soft-kmeans-la-def}).
Let us also consider all definitions of ``$\lambda$'' across all namespaces:
there are 69 occurrences of ``$\lambda$'' (see table~\ref{tab:strong-kmeans-lambda}). 
