...
class Evaluator:
    ...
    def _def_docfreq_calculate(self, doc_ids_definitions):
        ...
        #doc_df_definitions = {d: np.log(s + 1) for d, s in docfreq_definitions.items()}
        # self.doc_df_definitions = doc_df_definitions

        #N = len(self.doc_titles) * 1.0
        #doc_idf_definitions = {d: np.log(N / (df + 1)) for d, df in doc_df_definitions.items()}
        #self.doc_idf_definitions = doc_idf_definitions
